...
export default async function OSYPage() {
  const user = await getCurrentUser();
  if (!user) redirect("/login");
  const list = await monitoringList(user, "osy");

  return (
    <div className="space-y-6">
      <div className="flex items-center gap-2">
        <Link href="/monitoring" className="text-brand-500 hover:text-brand-800"><ArrowLeft className="h-4 w-4" /></Link>
        <h1 className="text-xl font-extrabold text-brand-900">{MONITORING_LABELS.osy}</h1>
      </div>
      <div className="rounded-xl border border-brand-200 bg-white shadow-sm overflow-hidden">
        <table className="w-full text-sm">
          ...
          <tbody className="divide-y divide-brand-100">
            {list.length === 0 ? (
              <tr><td colSpan={5} className="px-4 py-8 text-center text-brand-500">No out-of-school youth records found.</td></tr>
            ) : list.map((item) => (
              <tr key={item.id} className="hover:bg-brand-50">
                ...
                <td className="px-4 py-3">{new Date().getFullYear() - parseInt(item.birthDate.slice(0,4))}</td>
