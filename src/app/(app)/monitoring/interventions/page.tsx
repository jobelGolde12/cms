...
export default async function InterventionsPage() {
  ...
  const list = await monitoringList(user, "intervention");

  return (
    <div className="space-y-6">
      <div className="flex items-center gap-2">
        <Link href="/monitoring" className="text-brand-500 hover:text-brand-800"><ArrowLeft className="h-4 w-4" /></Link>
        <h1 className="text-xl font-extrabold text-brand-900">{MONITORING_LABELS.intervention}</h1>
      </div>
      <div className="rounded-xl border border-brand-200 bg-white shadow-sm overflow-hidden">
        <table className="w-full text-sm">
          ...
          <tbody className="divide-y divide-brand-100">
            {list.length === 0 ? (
              <tr><td colSpan={4} className="px-4 py-8 text-center text-brand-500">No intervention records found.</td></tr>
            ) : list.map((item) => (
